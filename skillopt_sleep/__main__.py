...
def _report_payload(rep, outcome) -> Dict[str, Any]:
    return {
        ...
        "llm_uncheckable_candidates": rep.llm_uncheckable_candidates,
        ...
        "n_pending_tasks": rep.n_pending_tasks,
        "harvest_cursor_advanced": rep.harvest_cursor_advanced,
...
def _add_common(p: argparse.ArgumentParser) -> None:
    p.add_argument("--project", default="", help="project used for state, staging, and live memory")
    p.add_argument("--scope", default="", choices=["", "all", "invoked"])
    p.add_argument("--scope-project", default="",
...
def _cfg_from_args(args, task_meta: Dict[str, Any] | None = None) -> Any:
    ...
    if getattr(args, "scope_project", ""):
        overrides["scope_project"] = os.path.abspath(args.scope_project)
    if args.backend:
