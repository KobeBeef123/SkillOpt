...
from skillopt_sleep.mine import assign_splits, count_checkable_tasks, dedup_tasks, mine
...
def _harvest_cursor_key(cfg: SleepConfig, project: str) -> str:
    source = str(cfg.get("transcript_source", "claude"))
    scope = cfg.get("projects", "invoked")
    if scope == "invoked":
        scope_value = os.path.abspath(cfg.get("scope_project") or project)
        if source == "claude" and scope_value == os.path.abspath(project):
            return project  # preserve existing state keys for the legacy default
    elif isinstance(scope, (list, tuple)):
        scope_value = ";".join(sorted(os.path.abspath(str(path)) for path in scope))
    else:
        scope_value = str(scope)
    return f"{project}|{source}|{scope_value}"
...
def _default_miner_diagnostics() -> Dict[str, Any]:
    return {
        ...
        "llm_uncheckable_candidates": 0,
        "llm_miner_failed": False,
        "llm_miner_error": "",
        "fallback_used": False,
        "n_checkable_tasks": 0,
...
def _apply_miner_diagnostics(report: SleepReport, diagnostics: Dict[str, Any]) -> None:
    ...
    report.llm_uncheckable_candidates = int(
        diagnostics.get("llm_uncheckable_candidates", 0) or 0
    )
...
def _finish_without_consolidation(
    ...
    *,
    ...
    tokens_used: int,
    advance_cursor: bool,
    pending_tasks: Optional[List[TaskRecord]] = None,
) -> CycleOutcome:
    report.n_replayed = 0
    ...
    if not dry_run:
        if pending_tasks is not None:
            state.set_pending_tasks(cursor_key, [task.to_dict() for task in pending_tasks])
        if advance_cursor:
            state.set_last_harvest(cursor_key, started)
            report.harvest_cursor_advanced = True
        ...
        state.record_night({
            ...
            "n_pending_tasks": report.n_pending_tasks,
            "harvest_cursor_advanced": report.harvest_cursor_advanced,
            "staging": staging_dir,
        })
        state.save()
...
def _render_report_md(report: SleepReport, cfg: SleepConfig) -> str:
    ...
    lines.extend([
        ...
        f"- uncheckable candidates dropped: {report.llm_uncheckable_candidates}",
        ...
        f"- pending checkable tasks: {report.n_pending_tasks}",
        f"- harvest cursor advanced: {report.harvest_cursor_advanced}",
        "",
    ])
...
def run_sleep_cycle(
    ...
    *,
    ...
    cursor_key = _harvest_cursor_key(cfg, project)
    ...
    all_pending_tasks: List[TaskRecord] = []
    deferred_tasks: List[TaskRecord] = []
    if seed_tasks is not None:
        ...
        since = state.last_harvest_for(cursor_key)
        ...
        mining_failed = bool(
            miner_diagnostics.get("llm_miner_failed")
            or miner_diagnostics.get("miner_mode") == "llm_unavailable"
        )
        if real_backend:
            pending = [
                TaskRecord.from_dict(task)
                for task in state.pending_tasks_for(cursor_key)
            ]
            all_pending_tasks = dedup_tasks(pending + tasks)
            tasks = all_pending_tasks[:max_tasks]
            deferred_tasks = all_pending_tasks[max_tasks:]
            tasks = assign_splits(
                tasks,
                holdout_fraction=cfg.get("holdout_fraction", 0.34),
                seed=cfg.get("seed", 42),
    ...
    if not tasks:
        ...
        if backend_name != "mock":
            ...
            report.notes.append(
                "harvest cursor retained for retry"
                if mining_failed
                else "valid empty mining result consumed; harvest cursor advanced"
            )
        return _finish_without_consolidation(
            ...
            advance_cursor=seed_tasks is None and not mining_failed,
            pending_tasks=[] if seed_tasks is None and not mining_failed else None,
        )
    ...
    if gate_reason:
        ...
        report.n_pending_tasks = len(all_pending_tasks) if seed_tasks is None else 0
        if seed_tasks is None:
            report.notes.append(
                f"retained {len(all_pending_tasks)} checkable tasks for the next successful harvest"
            )
            if mining_failed:
                report.notes.append("harvest cursor retained because mining did not complete cleanly")
        return _finish_without_consolidation(
            cfg,
            report=report,
            state=state,
            ...
            tokens_used=backend.tokens_used(),
            advance_cursor=seed_tasks is None and not mining_failed,
            pending_tasks=all_pending_tasks if seed_tasks is None else None,
        )
    ...
    if not dry_run:
        ...
        report.harvest_cursor_advanced = seed_tasks is None and not mining_failed
        report.n_pending_tasks = len(deferred_tasks) if seed_tasks is None else 0
        report_md = _render_report_md(report, cfg)
        ...
        try:
            ...
            pass
        if seed_tasks is None:
            state.set_pending_tasks(
                cursor_key, [task.to_dict() for task in deferred_tasks]
            )
